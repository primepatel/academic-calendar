...
def gen_holi(title, lst, x):
  ...
  for date, holi in lst:
    text += f"""
    <text x="50" y="{m}" class="date">{calendar.month_name[int(date.split(" ")[1])][:3] + " " + date.split(" ")[0]}</text>
...
with open("main1.svg", "w") as svgfile:
    svgfile.write(file)
    
from cairosvg import svg2png

svg2png(url="main1.svg", write_to="main1.png", output_height=1080)